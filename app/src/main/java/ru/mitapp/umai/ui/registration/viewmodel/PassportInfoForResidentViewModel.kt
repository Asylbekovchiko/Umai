...
class PassportInfoForResidentViewModel : ViewModel() {
    var isButtonActive = ObservableField(false)


    fun checkInputs(documentName: String?, passportNumber: String?,
                    dateIssue: String?, expirationDate: String?, whoIssued: String) {

        if(!documentName.isNullOrEmpty() && !passportNumber.isNullOrEmpty()
